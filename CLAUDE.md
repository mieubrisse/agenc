...
Every function in `cmd/` that emits an ANSI escape carries `ForPicker` in its name: `displayGitRepoForPicker`, `colorizeStatusForPicker`, `attachedDotForPicker`, `formatMissionCellForPicker`, `formatPaletteEntryLineForPicker`, `formatUnreadBannerForPicker`. Two more return colour by delegation rather than by emitting it — `formatRepoDisplayForPicker`, and the shared body `buildRepoDisplay`, which inherits whatever the formatter it is handed returns and is reached only through its two wrappers.

Within `cmd/`, then, an unsuffixed name returns plain text. Keep it that way when you add one: the suffix is the only warning a future agent gets, and a colour-returning helper without it is a trap, because the convention's own signal inverts and the name promises safety it does not deliver. (Outside `cmd/`, `database.ColorizeSnippet` returns colour under a self-describing name, paired with `database.StripSnippetMarkers` for command output.)
