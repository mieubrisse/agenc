...
- `session_registry.go` — `ListMissionSessions` reads Claude Code's live-session registry (`~/.claude/sessions/<pid>.json`, one record per running session) and returns the records whose working directory is a mission's `agent/` directory, keyed to that mission. The registry is the same one behind Claude Code's `ListAgents` tool, so the peer name and tmux target it yields are the strings an agent sees there. This file is the only place in AgenC that knows the registry's shape; unreadable or malformed records are skipped rather than failing the read, and liveness filtering is left to the caller.
...
Missions are denied write access to the repo library directory via an injected `Edit` deny permission in settings.json (`internal/claudeconfig/overrides.go`); the canonical `Edit` verb covers all file-editing tools (Write/Edit/NotebookEdit). Read access is intentionally left allowed so agents can explore code in other repos without spawning a new mission.
