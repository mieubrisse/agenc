...
**The `claude-modifications` layers must be re-homed** (they were merged into the snapshot today): `claude-modifications/settings.json` → deep-merge into the op-settings file (Increment 1 already does this); `claude-modifications/CLAUDE.md` → fold into `agenc prime`'s output (or `--append-system-prompt`), since State Y does no CLAUDE.md file merge. Confirm whether Kevin actually uses `claude-modifications` (open question Q2).
...
- **Q2 — does Kevin use `claude-modifications`?** Determines how much care the CLAUDE.md/settings re-home (§5) needs. *Flag for Kevin if non-trivial content exists.*
- **Q3 — devcontainer fate / sequencing (C5).** *Recommend agenc-ok7h first; confirm no devcontainer missions in flight — Kevin.*
