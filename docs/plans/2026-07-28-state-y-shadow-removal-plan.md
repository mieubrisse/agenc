...
**REFINED by Kevin (2026-07-31): token injection becomes opt-in, gated on the token file's presence — not ripped out.** Keep the machine-token path but make it conditional, so the default is native auth and the machine token is a runtime fallback.

- **`BuildClaudeCmd` (`mission.go`):** always unset `CLAUDE_CONFIG_DIR` (shadow removal). For the token: read the token file; if present and non-empty, inject `CLAUDE_CODE_OAUTH_TOKEN` (State X: auth stable, connectors broken); if absent, inject **nothing** and let Claude use its native `~/.claude` auth (State Y: connectors work). **Remove the current "no OAuth token configured" error** (`mission.go:106-114`) — an absent token is now the valid default, not a failure.
- **`session_summarizer.go` already does this** (`if oauthToken != "" { inject }`, and `ReadOAuthToken` returns `""` not an error when absent) — leave it; it's conformant by construction.
- **Default = native Claude auth** (no token file) → connectors work; what new users get out of the box.
- **Opt-in fallback = the machine token**, for when the native-auth refresh race proves intolerable at high mission concurrency (agenc-lm3d). Writing a token flips a fleet back to State X with no code change — a runtime toggle, superseding the old "keep the key dormant / one-line re-add" plan (Q4).

**New CLI surface: `agenc token set <token>` / `agenc token clear`** — dedicated commands over the token cache file (cleaner than `agenc config set claudeCodeOAuthToken`). `clear` deletes the file → native auth; `set` writes it → machine token. `/config-key-checklist` governs the CLI/README/arch-doc updates; decide whether to keep the `claudeCodeOAuthToken` config key as an alias or retire it in favor of `agenc token`.

**Onboarding:** no forced `claude setup-token`. Replace `SetupOAuthToken`'s forced-token flow: if a token file is present, use it; otherwise rely on native `~/.claude` auth. For **headless** (no TTY, can't prompt), verify *some* auth exists (token file OR a native `claudeAiOauth` credential in the keychain / `~/.claude.json`) and **fail loudly** with guidance (`claude auth login`, or `agenc token set`) if neither does. Interactive missions can let Claude's own native login prompt handle a logged-out state.

**Prerequisite (Kevin, 2026-07-31): resolved via self-test.** Kevin is OK with AgenC writing the trust entry into the real `~/.claude.json` (atomic temp+rename, verify-retry per §4) and will run State Y himself under real concurrency to empirically check whether the refresh race / concurrent-write corruption is actually problematic before rolling it wider. The token toggle above is the escape hatch if it is. (Supersedes the "needs Kevin's go-ahead" gate on #1; #3 native-login is now just "have `claude auth login` done, or `agenc token set` a machine token.")
...
- **Q4 — token fallback: SUPERSEDED by Kevin's 2026-07-31 refinement (see §6).** Not "dormant code" but a live **runtime toggle**: token injection is conditional on the token cache file's presence, managed by new `agenc token set/clear` CLI. File absent = native auth (default, State Y, connectors work); file present = machine token (State X fallback). No code change to switch.
