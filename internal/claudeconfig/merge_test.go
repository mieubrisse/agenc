...
func TestMergeAgencSandboxFilesystem_NoAdditionsIsNoop(t *testing.T) {
	settingsJSON := `{"sandbox":{"filesystem":{"allowWrite":["//tmp"]}}}`
	var settings map[string]json.RawMessage
	if err := json.Unmarshal([]byte(settingsJSON), &settings); err != nil {
		t.Fatalf("failed to parse test settings: %v", err)
	}

	if err := mergeAgencSandboxFilesystem(settings, nil); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	allowWrite := extractSandboxAllowWrite(t, settings)
	if len(allowWrite) != 1 || allowWrite[0] != "//tmp" {
		t.Fatalf("expected existing settings unchanged, got %v", allowWrite)
	}
}

func TestMergeAgencSandboxFilesystem_EmptySettings(t *testing.T) {
	settings := make(map[string]json.RawMessage)
	pathToAdd := "/var/folders/aa/bbb/T/**"

	if err := mergeAgencSandboxFilesystem(settings, []string{pathToAdd}); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	allowWrite := extractSandboxAllowWrite(t, settings)
	if len(allowWrite) != 1 || allowWrite[0] != pathToAdd {
		t.Fatalf("expected [%s], got %v", pathToAdd, allowWrite)
	}
}

func TestMergeAgencSandboxFilesystem_AppendsToExisting(t *testing.T) {
	settingsJSON := `{"sandbox":{"filesystem":{"allowWrite":["//tmp","~/Library/Caches/go-build"]}}}`
	var settings map[string]json.RawMessage
	if err := json.Unmarshal([]byte(settingsJSON), &settings); err != nil {
		t.Fatalf("failed to parse test settings: %v", err)
	}
	...
	allowWrite := extractSandboxAllowWrite(t, settings)
	expected := []string{"//tmp", "~/Library/Caches/go-build", pathToAdd}
	if len(allowWrite) != len(expected) {
		t.Fatalf("expected %d entries, got %d: %v", len(expected), len(allowWrite), allowWrite)
	}
	for i, want := range expected {
		if allowWrite[i] != want {
			t.Errorf("index %d: expected %q, got %q", i, want, allowWrite[i])
		}
	}
}

func TestMergeAgencSandboxFilesystem_NoDuplicate(t *testing.T) {
	pathToAdd := "/var/folders/aa/bbb/T/**"
	settingsJSON := `{"sandbox":{"filesystem":{"allowWrite":["` + pathToAdd + `"]}}}`
	var settings map[string]json.RawMessage
	if err := json.Unmarshal([]byte(settingsJSON), &settings); err != nil {
		t.Fatalf("failed to parse test settings: %v", err)
	}
	...
	allowWrite := extractSandboxAllowWrite(t, settings)
	if len(allowWrite) != 1 {
		t.Fatalf("expected 1 entry (no duplicate), got %d: %v", len(allowWrite), allowWrite)
	}
}

func TestMergeAgencSandboxFilesystem_PreservesAdjacentSandboxFields(t *testing.T) {
	settingsJSON := `{"sandbox":{"enabled":true,"network":{"allowUnixSockets":["//tmp/claude"]},"filesystem":{"denyWrite":["//etc/passwd"]}}}`
	var settings map[string]json.RawMessage
	if err := json.Unmarshal([]byte(settingsJSON), &settings); err != nil {
		t.Fatalf("failed to parse test settings: %v", err)
	}

	if err := mergeAgencSandboxFilesystem(settings, []string{"/var/folders/aa/bbb/T/**"}); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	if _, ok := sandboxMap["enabled"]; !ok {
		t.Error("expected sandbox.enabled to be preserved")
	}
	if _, ok := sandboxMap["network"]; !ok {
		t.Error("expected sandbox.network to be preserved")
	}

	var fsMap map[string]json.RawMessage
	if err := json.Unmarshal(sandboxMap["filesystem"], &fsMap); err != nil {
		t.Fatalf("failed to parse filesystem: %v", err)
	}
	if _, ok := fsMap["denyWrite"]; !ok {
		t.Error("expected sandbox.filesystem.denyWrite to be preserved")
	}
	if _, ok := fsMap["allowWrite"]; !ok {
		t.Error("expected sandbox.filesystem.allowWrite to be added")
	}
}

// extractSandboxAllowWrite is a test helper that extracts the
// sandbox.filesystem.allowWrite array from a settings map.
func extractSandboxAllowWrite(t *testing.T, settings map[string]json.RawMessage) []string {
	t.Helper()
	...
	var fsMap map[string]json.RawMessage
	if err := json.Unmarshal(sandboxMap["filesystem"], &fsMap); err != nil {
		t.Fatalf("failed to parse sandbox.filesystem: %v", err)
	}

	var allowWrite []string
	if err := json.Unmarshal(fsMap["allowWrite"], &allowWrite); err != nil {
		t.Fatalf("failed to parse sandbox.filesystem.allowWrite: %v", err)
	}
