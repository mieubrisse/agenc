...
When you spawn a child mission (`agenc mission new <repo> --prompt "..."`), the child has no shared conversation history with you. Frame the **problem**, not the **procedure**: state the goal, link the relevant files / skills / beads / prior sessions, name what's been tried, point at the skill that owns the workflow if one is obvious. Numbered "FIRST ACTION: read X. THEN: do Y" step-lists trap the child inside your improvised plan and short-circuit the skill that would have served better — same failure mode this rule prevents recursively applies to bead descriptions and any other handoff to an agent who arrives without your context.
