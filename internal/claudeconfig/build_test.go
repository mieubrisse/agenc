...
func TestSymlinkToGlobalClaudeFile_CreatesSymlinkAndSeedsTarget(t *testing.T) {
	homeDir := setupFakeHome(t)
	...
	if err := symlinkToGlobalClaudeFile(claudeConfigDirpath, "mcp-needs-auth-cache.json"); err != nil {
		t.Fatalf("symlinkToGlobalClaudeFile failed: %v", err)
	}

	linkPath := filepath.Join(claudeConfigDirpath, "mcp-needs-auth-cache.json")
	info, err := os.Lstat(linkPath)
	if err != nil {
		t.Fatalf("expected link at %q: %v", linkPath, err)
	}
	if info.Mode()&os.ModeSymlink == 0 {
		t.Errorf("expected %q to be a symlink, mode=%v", linkPath, info.Mode())
	}
	...
	expectedTarget := filepath.Join(homeDir, ".claude", "mcp-needs-auth-cache.json")
	if target != expectedTarget {
		t.Errorf("symlink target = %q, want %q", target, expectedTarget)
	}

	// Target must exist (seeded with "{}" if absent), so reads through the link succeed.
	contents, err := os.ReadFile(linkPath)
	if err != nil {
		t.Fatalf("read through symlink failed: %v", err)
	}
	if string(contents) != "{}" {
		t.Errorf("expected seeded target to contain {}, got %q", string(contents))
	}
}

func TestSymlinkToGlobalClaudeFile_PreservesExistingTargetContents(t *testing.T) {
	homeDir := setupFakeHome(t)
	claudeConfigDirpath := filepath.Join(homeDir, ".agenc", "missions", "test-mission", "claude-config")
	if err := os.MkdirAll(claudeConfigDirpath, 0755); err != nil {
		t.Fatal(err)
	...
	existing := `{"composio":{"timestamp":12345}}`
	targetFilepath := filepath.Join(homeDir, ".claude", "mcp-needs-auth-cache.json")
	if err := os.WriteFile(targetFilepath, []byte(existing), 0644); err != nil {
		t.Fatal(err)
	}
	...
	contents, err := os.ReadFile(targetFilepath)
	if err != nil {
		t.Fatalf("read target failed: %v", err)
	}
	if string(contents) != existing {
		t.Errorf("expected target contents preserved, got %q", string(contents))
	}
}

func TestSymlinkToGlobalClaudeFile_ReplacesExistingRegularFile(t *testing.T) {
	homeDir := setupFakeHome(t)
	claudeConfigDirpath := filepath.Join(homeDir, ".agenc", "missions", "test-mission", "claude-config")
	if err := os.MkdirAll(claudeConfigDirpath, 0755); err != nil {
		t.Fatal(err)
	...
	linkPath := filepath.Join(claudeConfigDirpath, "mcp-needs-auth-cache.json")
	if err := os.WriteFile(linkPath, []byte(`{"stale":{"timestamp":1}}`), 0644); err != nil {
		t.Fatal(err)
	}
	...
	if info.Mode()&os.ModeSymlink == 0 {
		t.Errorf("expected stale regular file to be replaced with a symlink, mode=%v", info.Mode())
	}
}
