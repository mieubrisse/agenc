...
func TestSeedMcpNeedsAuthCache_CopiesFromGlobal(t *testing.T) {
	homeDir := setupFakeHome(t)
	if err := os.MkdirAll(filepath.Join(homeDir, ".claude"), 0700); err != nil {
		t.Fatal(err)
	}
	srcContent := `{"composio":{"timestamp":42},"posthog":{"timestamp":99}}`
	srcFilepath := filepath.Join(homeDir, ".claude", "mcp-needs-auth-cache.json")
	if err := os.WriteFile(srcFilepath, []byte(srcContent), 0644); err != nil {
		t.Fatal(err)
	}

	claudeConfigDirpath := filepath.Join(homeDir, ".agenc", "missions", "test-mission", "claude-config")
	if err := os.MkdirAll(claudeConfigDirpath, 0755); err != nil {
		t.Fatal(err)
	}

	if err := seedMcpNeedsAuthCache(claudeConfigDirpath); err != nil {
		t.Fatalf("seedMcpNeedsAuthCache failed: %v", err)
	}

	destFilepath := filepath.Join(claudeConfigDirpath, "mcp-needs-auth-cache.json")
	got, err := os.ReadFile(destFilepath)
	if err != nil {
		t.Fatalf("read dest failed: %v", err)
	}
	if string(got) != srcContent {
		t.Errorf("dest contents = %q, want %q", string(got), srcContent)
	}

	// Must be a regular file (NOT a symlink) — Claude replaces symlinks on write,
	// which would lose any per-mission updates the user makes during the session.
	info, err := os.Lstat(destFilepath)
	if err != nil {
		t.Fatalf("Lstat failed: %v", err)
	}
	if info.Mode()&os.ModeSymlink != 0 {
		t.Errorf("expected regular file, got symlink: mode=%v", info.Mode())
	}
}

func TestSeedMcpNeedsAuthCache_SeedsEmptyWhenGlobalMissing(t *testing.T) {
	homeDir := setupFakeHome(t)
	// Intentionally do NOT create ~/.claude/mcp-needs-auth-cache.json.
	...
	got, err := os.ReadFile(filepath.Join(claudeConfigDirpath, "mcp-needs-auth-cache.json"))
	if err != nil {
		t.Fatalf("read dest failed: %v", err)
	}
	if string(got) != "{}" {
		t.Errorf("expected fallback '{}', got %q", string(got))
	}
}

func TestSeedMcpNeedsAuthCache_ReplacesExistingFile(t *testing.T) {
	homeDir := setupFakeHome(t)
	if err := os.MkdirAll(filepath.Join(homeDir, ".claude"), 0700); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(homeDir, ".claude", "mcp-needs-auth-cache.json"), []byte(`{"a":{"timestamp":1}}`), 0644); err != nil {
		t.Fatal(err)
	}
	...
	// Pre-existing stale per-mission file
	destFilepath := filepath.Join(claudeConfigDirpath, "mcp-needs-auth-cache.json")
	if err := os.WriteFile(destFilepath, []byte(`{"stale":{"timestamp":0}}`), 0644); err != nil {
		t.Fatal(err)
	}
	...
	if string(got) != `{"a":{"timestamp":1}}` {
		t.Errorf("expected source contents to replace stale file, got %q", string(got))
	}
}
